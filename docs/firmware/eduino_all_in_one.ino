...
//         LED(불) · MOD(수동/자율) · PRM(실시간 튜닝) · PNG(하트비트)
//   텔레메트리: DST(초음파 거리) · LIN(IR 라인센서) 주기 송신 → 자율주행 화면에 표시
//
...
void handle(String line) {
  ...
  if (cmd == "DRV") {
    ...
    digitalWrite(LED_PIN, arg.startsWith("1") ? HIGH : LOW);
  } else if (cmd == "STP") {
    stopAll();
  } else if (cmd == "MOD") {
    autoMode = (arg == "AUTO");
    if (!autoMode) stopAll();
    bt.print("MOD:"); bt.println(autoMode ? "AUTO" : "MANUAL"); // 앱에 응답
  } else if (cmd == "PRM") {
    int comma = arg.indexOf(',');
...
void runAuto() {
#if ENABLE_LINE
  // 간단 라인트레이싱(검은 선 = 낮은 값 가정, 임계=lineSens 로 조정)
  int l = analogRead(IR_L), c = analogRead(IR_C), r = analogRead(IR_R);
  int th = map(lineSens, 0, 100, 40, 90); // 민감도→기본 속도
  bool onL = l < 512, onC = c < 512, onR = r < 512;
  if (onC) drive(th, 0);
  else if (onL) drive(th, -60);
  else if (onR) drive(th, 60);
  else drive(th, 0);
  return;
#endif
#if ENABLE_ULTRASONIC
